...
; Version 3, 23 October 2007: changed albedo for areas identified as 'ice'; now on 'ppm' sandbox
; Version 3.1, 26 October 2007: correct points with with total PFT fraction equal to 0

; IDL programme to read bucket size information of full BETHY simulations
; and generate an input file for CCDAS
...
IMBETHY_infile = '/Users/wolfgang/Models/CCDAS/ppm/control_bethy/adbethy_grid3veg_79-00.nc'
newCCDAS_outfile = '/Users/wolfgang/Models/CCDAS/ppm/control_bethy/bethy_grid3veg_v2.nc'
...
; minimum wmax value [mm]:
wmaxmin = 10.
loadm, fullBETHYfile, v, format=fullBETHYformat, gridwidth=2.0, /center, co=20,oc=missing
wmax = v[*,*,18]
land = wmax NE missing
wmax = (wmax > wmaxmin)*land + missing * (land EQ 0)
...
rhosw = rhosw * land + missing * (land eq 0)
rhosd = rhosd * land + missing * (land eq 0)
...
des = (5.62 - 2.56 * sand1) * land + missing * (land eq 0)
evap1 = (14.29 - 9.23 * sand1) * land + missing * (land eq 0)
...
window,0 & tvf,wmax,/bar,/map,missing=missing,title='max plant avail soil water [mm]'
window,1 & tvf,rhosw,/bar,missing=missing,/map,title='wet soil albedo'
window,2 & tvf,rhosd,/bar,missing=missing,/map,title='dry soil albedo'
window,3 & tvf,des,/bar,missing=missing,/map,title='desorptivity [mm/sqrt(day)]'
window,4 & tvf,evap1,/bar,missing=missing,/map,title='phase 1 evaporation [mm]'
...
vmtype = NCDF_VARDEF(id, 'mtype', [dlon, dlat], /LONG)
NCDF_ATTPUT, id, vmtype, 'long_name', 'Single vegetation type (0-13)', /CHAR
NCDF_ATTPUT, id, vmtype, 'units', '', /CHAR
NCDF_ATTPUT, id, vmtype, 'missing_value', '-9999', /LONG
...
NCDF_VARPUT, id, vmtype, in.mtype
NCDF_VARPUT, id, vtype, in.type
NCDF_VARPUT, id, vfrac, in.frac
NCDF_VARPUT, id, vwmax, wmax
NCDF_VARPUT, id, vrhosw, rhosw
NCDF_VARPUT, id, vrhosd, rhosd
NCDF_VARPUT, id, vdes, des
NCDF_VARPUT, id, vevap1, evap1
