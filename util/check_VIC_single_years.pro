PRO check_VIC, name=name, smin = smin

path = '/Users/wolfgang/Models/VIC_drivers/revision_june_2008/'
missing = 1e20
year0 = 1979
year1 = 2005
device,decompose=0
...
ncdf_read,field0,file=path+'forcing.2x2.'+name+'.'+strcompress(year,/rem)+'.nc',variables=name,/no_struct
s0 = size (field0) & nx = s0[1] & ny = s0[2] & nt0 = s0[3]

IF year EQ year0 THEN BEGIN
  field = field0
  s = size (field) & nx = s[1] & ny = s[2] & nt = s[3]
ENDIF ELSE BEGIN
  field1 = field
  field = fltarr(nx,ny,nt+nt0)
  field[*,*,0:nt-1] = field1
  field[*,*,nt:nt+nt0-1] = field0
  nt = nt + nt0
ENDELSE
