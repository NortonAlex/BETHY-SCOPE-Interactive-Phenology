...
program prgtstjacvecgtb
  ...
  REAL    eps, acc
  integer nbeg, nend, nstep, jtest
  namelist /check/ eps, acc, nbeg, nend, nstep, jtest
  character*(*) cfile
  parameter ( cfile = 'tstgtb.par')
  integer i,ncol
...
!==============================================================
      call doit( n, m, ncol, nbeg, nend, nstep, jtest, acc, eps, epsmach  )

end program prgtstjacvecgtb
...
subroutine doit( n, m, ncol, nbeg, nend, nstep, jtest, acc, eps, epsmach )
      use mo_timing
      implicit none
...
! compute normalised difference
            rel(icol,j) = abs(gfd(icol,j)-adx(col(icol)))
            absmax = max( abs(gfd(icol,j)), abs(adx(col(icol))) )
...
! print result of test
            if (rel(icol,j) .lt. acc) then
               print '(2i6,4(1x,e12.6)  )',col(icol),j,x(col(icol)),gfd(icol,j),adx(col(icol)),rel(icol,j)
            else
               print '(2i6,4(1x,e12.6),a)',col(icol),j,x(col(icol)),gfd(icol,j),adx(col(icol)),rel(icol,j), &
                    ' DIFFERENT'
            endif
         end do
      enddo
...
!===============================================
      ...
      print '(a,i8)'   , ' based on adjoint calls       : ', ncgrad
      print '(a,i8)'   , ' no. of columns               : ', ncol
      print '(a,f12.5)', ' run time function            : ', tfunc
      print '(a,f12.5)', ' run time adjoint             : ', tgrad
