...
!   All rights reserved.
!**************************************************************
program prgtstjaccol
  implicit none
  integer :: n, m
  REAL        epsmach
  parameter ( epsmach = 1.E-16 )
  REAL        epsbeg
...
!==============================================================
      call doit( n, m, ncol, nbeg, nend, nstep, acc, eps, epsmach  )

end program prgtstjaccol
...
subroutine doit( n, m, ncol, nbeg, nend, nstep, acc, eps, epsmach )
      use mo_timing
      implicit none
...
! compute normalised difference
            rel(icol,j) = abs(gfd(icol,j)-g_y(icol,j))
            absmax = max( abs(gfd(icol,j)), abs(g_y(icol,j)) )
...
! print result of test
            if (rel(icol,j) .lt. acc) then
               print '(2i6,4(1x,e12.6)  )',col(icol),j,x(col(icol)),gfd(icol,j),g_y(icol,j),rel(icol,j)
            else
               print '(2i6,4(1x,e12.6),a)',col(icol),j,x(col(icol)),gfd(icol,j),g_y(icol,j),rel(icol,j), &
                    ' DIFFERENT'
            endif
         end do
      enddo
